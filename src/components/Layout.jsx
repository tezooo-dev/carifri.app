    ...
    <div className="flex h-screen overflow-hidden bg-slate-100">
      ...
      `}>
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-700/60">
          ...
          <button onClick={() => setOpen(false)} className="ml-auto lg:hidden text-slate-400 hover:text-white shrink-0">
            <X size={16} />
          </button>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="bg-white border-b border-slate-200 px-4 py-3 flex items-center gap-3 lg:px-6">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-xs text-slate-400">Welcome back{user ? `, ${user.name.split(' ')[0]}` : ''}</div>
          ...
          <div className="hidden sm:flex items-center gap-4 text-xs text-slate-500">
            ...
            <span>{settings.currency} {stats.receivedCommission.toLocaleString()} earned</span>
          </div>
          {/* Notification bell */}
          <NotificationCenter />
          {/* Post load shortcut */}
